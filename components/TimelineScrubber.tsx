...
export const TimelineScrubber: React.FC<TimelineScrubberProps> = ({ rectors, currentIndex, onScrollTo }) => {
  ...
  return (
    <div className={`absolute left-0 top-0 bottom-0 w-12 z-50 flex items-center justify-center pointer-events-none transition-opacity duration-[250ms] ${showTimeline ? 'opacity-100' : 'opacity-0'}`}>
          ...
          return (
            ...
            >
              ...
              <div className="relative flex items-center justify-center">
                ...
                <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-white/90 text-black text-[10px] font-bold px-2 py-0.5 rounded opacity-0 transition-opacity duration-[100ms] pointer-events-none whitespace-nowrap ${isActive ? 'opacity-100' : 'group-hover:opacity-100'}`}>
                ...
                {/* Dot */}
                <div
                  className={`rounded-full transition-all duration-[150ms] ${isActive ? 'w-2.5 h-2.5 bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.8)]' : 'w-1.5 h-1.5 bg-white/30 hover:bg-white/60'}`}
                />
              </div>
            </div>
          );
